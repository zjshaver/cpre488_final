...
// Dependencies:        
//
// Revision:            Sep 01, 2011: 1.00 Initial version
//                      Oct 07, 2011: 1.01 Fix bug with I2C I/O expander code
...
int fmc_imageon_init( fmc_imageon_t *pContext, char szName[], fmc_iic_t *pIIC )
{
   ...
   fmc_imageon_iic_mux_reset( pContext );

#if 1
   // Initialize the I/O Expander to default state
   fmc_imageon_iic_mux( pContext, FMC_IMAGEON_I2C_SELECT_IO_EXP );

   // Configure direction of I/O Expander's Px GPIO pins
   reg_addr = 0x03;
   //num_bytes = pContext->pIIC->fpIicRead( pContext->pIIC, FMC_IMAGEON_IO_EXP_ADDR, reg_addr, &reg_data, 1); 
   //xil_printf( "[fmc_imageon_init] PCA9534[0x%02X] <= 0x%02X\n\r", reg_addr, reg_data );
   reg_data = 0xEA; // P0 => HDMII_RST#  = 0 (output)
                    ...
                    // P7 => unused      = 1 (input)
   //xil_printf( "[fmc_imageon_init] PCA9534[0x%02X] <= 0x%02X\n\r", reg_addr, reg_data );
   num_bytes = pContext->pIIC->fpIicWrite( pContext->pIIC, FMC_IMAGEON_IO_EXP_ADDR, reg_addr, &reg_data, 1); 
...
{   
   ...
   //  [0] = fmc_imageon_iic_rst# = 0
   pContext->GpioData &= ~0x01;
   pContext->pIIC->fpGpoWrite( pContext->pIIC, pContext->GpioData );
   
   // sleep 1sec
   fmc_imageon_wait_usec(1000000); 
   
   // Release reset from I2C mux
   //xil_printf( "Release reset from I2C mux ...\n\r" );
   // Modify appropriate GPIO bits
   //  [0] = fmc_imageon_iic_rst# = 1
   pContext->GpioData |= 0x01;
   pContext->pIIC->fpGpoWrite( pContext->pIIC, pContext->GpioData );
}
...
*
******************************************************************************/
void fmc_imageon_iic_mux( fmc_imageon_t *pContext, Xuint32 MuxSelect )
{
   Xuint8 mux_data;
   Xuint8 num_bytes;
   ...
   switch ( MuxSelect )
   {
      ...
      //
      case FMC_IMAGEON_I2C_SELECT_DDCEDID:
      {
         mux_data = 0x01;
         num_bytes =  pContext->pIIC->fpIicWrite( pContext->pIIC, FMC_IMAGEON_I2C_MUX_ADDR, mux_data, &mux_data, 1); 
         break;
      }
      case FMC_IMAGEON_I2C_SELECT_HDMI_OUT:
      {
         mux_data = 0x02;
         num_bytes = pContext->pIIC->fpIicWrite( pContext->pIIC, FMC_IMAGEON_I2C_MUX_ADDR, mux_data, &mux_data, 1); 
         break;
      }
      case FMC_IMAGEON_I2C_SELECT_HDMI_IN:
      {
         mux_data = 0x04;
         num_bytes = pContext->pIIC->fpIicWrite( pContext->pIIC, FMC_IMAGEON_I2C_MUX_ADDR, mux_data, &mux_data, 1); 
         break;
      }
      case FMC_IMAGEON_I2C_SELECT_IO_EXP:
      //case FMC_IMAGEON_I2C_SELECT_VID_CLK:
      {
         mux_data = 0x08;
         num_bytes = pContext->pIIC->fpIicWrite( pContext->pIIC, FMC_IMAGEON_I2C_MUX_ADDR, mux_data, &mux_data, 1); 
         break;
      ...
      //
      case FMC_IMAGEON_I2C_SELECT_HDMI:
      {
         mux_data = 0x02 | 0x04;
         num_bytes = pContext->pIIC->fpIicWrite( pContext->pIIC, FMC_IMAGEON_I2C_MUX_ADDR, mux_data, &mux_data, 1); 
         break;
...
*
******************************************************************************/
void fmc_imageon_iic_config2( fmc_imageon_t *pContext, Xuint8 ChipAddress, 
                              Xuint8 ConfigData[][2], Xuint32 ConfigLength )
{
   ...
   {
      //xil_printf( "[fmc_imageon_iic_config2] IIC Write - Device = 0x%02X, Address = 0x%02X, Data = 0x%02X\n\r", ChipAddress<<1, ConfigData[i][0], ConfigData[i][1] );
                              ...
                              Xuint8 ConfigData[][3], Xuint32 ConfigLength )
{
   ...
   for ( i = 0; i < ConfigLength; i++ )
   {
      //xil_printf( "[fmc_imageon_iic_config3] IIC Write - Device = 0x%02X, Address = 0x%02X, Data = 0x%02X\n\r", ConfigData[i][0]<<1, ConfigData[i][1], ConfigData[i][2] );
...
void fmc_imageon_vclk_init( fmc_imageon_t *pContext )
{
   ...
   fmc_imageon_iic_mux( pContext, FMC_IMAGEON_I2C_SELECT_VID_CLK );
   ...
   for ( i = 0; i < MAX_IIC_CDCE913; i++ )
   {
      ...
      //xil_printf("[fmc_imageon_config_vclk] CDCE913[0x%02X] <= 0x%02X\n\r",(0x80 | iic_cdce913[i][0]), (iic_cdce913[i][1]));
...
*
******************************************************************************/
void fmc_imageon_vclk_config( fmc_imageon_t *pContext, Xuint32 FreqId )
{
   int dev;
   Xuint8 num_bytes;
   int i;
   ...
   switch ( FreqId )
   {
   case FMC_IMAGEON_VCLK_FREQ_25_175_000:
      for ( i = 0; i < 6; i++ )
      {
         num_bytes = pContext->pIIC->fpIicWrite( pContext->pIIC, FMC_IMAGEON_VID_CLK_ADDR, 
            (0x80 | iic_cdce913_y1_config_25_175_000[i][0]), &(iic_cdce913_y1_config_25_175_000[i][1]), 1); 
      }
      break;
   case FMC_IMAGEON_VCLK_FREQ_27_000_000:
      for ( i = 0; i < 6; i++ )
      {
         num_bytes = pContext->pIIC->fpIicWrite( pContext->pIIC, FMC_IMAGEON_VID_CLK_ADDR, 
      ...
      break;
   case FMC_IMAGEON_VCLK_FREQ_40_000_000:
      for ( i = 0; i < 6; i++ )
      {
         num_bytes = pContext->pIIC->fpIicWrite( pContext->pIIC, FMC_IMAGEON_VID_CLK_ADDR, 
      ...
      break;
   case FMC_IMAGEON_VCLK_FREQ_65_000_000:
      for ( i = 0; i < 6; i++ )
      {
         num_bytes = pContext->pIIC->fpIicWrite( pContext->pIIC, FMC_IMAGEON_VID_CLK_ADDR, 
      ...
      break;
   case FMC_IMAGEON_VCLK_FREQ_74_250_000:
      for ( i = 0; i < 6; i++ )
      {
         num_bytes = pContext->pIIC->fpIicWrite( pContext->pIIC, FMC_IMAGEON_VID_CLK_ADDR, 
      ...
      break;
   case FMC_IMAGEON_VCLK_FREQ_110_000_000:
      for ( i = 0; i < 6; i++ )
      {
         num_bytes = pContext->pIIC->fpIicWrite( pContext->pIIC, FMC_IMAGEON_VID_CLK_ADDR, 
      ...
      break;
   case FMC_IMAGEON_VCLK_FREQ_148_500_000:
      for ( i = 0; i < 6; i++ )
      {
         num_bytes = pContext->pIIC->fpIicWrite( pContext->pIIC, FMC_IMAGEON_VID_CLK_ADDR, 
      ...
      break;
   case FMC_IMAGEON_VCLK_FREQ_162_000_000:
      for ( i = 0; i < 6; i++ )
      {
         num_bytes = pContext->pIIC->fpIicWrite( pContext->pIIC, FMC_IMAGEON_VID_CLK_ADDR, 
...
Xuint8 iic_hdmi_in_config[IIC_HDMI_IN_CONFIG_LEN][3] =
{
      ...
      IIC_ADV7611_BASE_ADDR>>1, 0x19, 0x83, // LLC DLL phase
...
*
******************************************************************************/
int fmc_imageon_hdmii_init( fmc_imageon_t *pContext, Xuint32 Enable, Xuint32 edidInit, Xuint8 pEdid[256] )
{
   Xuint8 i2c_address;
   Xuint8 i2c_data;
   Xuint8 num_bytes;
   ...
   if ( !Enable )
   {
      ...
      fmc_imageon_hdmii_set_hpd( pContext, 0 );
      fmc_imageon_wait_usec(1000000);

      //xil_printf("\tAsserting Reset\n\r");
      fmc_imageon_hdmii_set_rst( pContext, 1 );
      fmc_imageon_wait_usec(1000000);
   }
   else
   {
      ...
      fmc_imageon_hdmii_set_rst( pContext, 0 );
      fmc_imageon_wait_usec(1000000);


#if 0
      //xil_printf( "I2C Initializing - Reset\n\r" );
      fmc_imageon_iic_mux( pContext, FMC_IMAGEON_I2C_SELECT_HDMI_IN );
      fmc_imageon_iic_config3( pContext, iic_hdmi_in_reset, IIC_HDMI_IN_RESET_LEN );
      fmc_imageon_wait_usec(1000000);
#endif

      //xil_printf("I2C Initializing - Mapping\n\r");
      fmc_imageon_iic_mux( pContext, FMC_IMAGEON_I2C_SELECT_HDMI_IN );
      fmc_imageon_iic_config3( pContext, iic_hdmi_in_mapping, IIC_HDMI_IN_MAPPING_LEN);
      if (edidInit)
      {
         fmc_imageon_hdmii_write_edid( pContext, pEdid );
      }
      fmc_imageon_iic_config3( pContext, iic_hdmi_in_edid_post, IIC_HDMI_IN_EDID_POST_LEN );

      // Now that the EDID has been programmed, allow HDMI source do "see" the HDMI receiver
      //xil_printf("\tAsserting HotPlugDetect\n\r");
      fmc_imageon_hdmii_set_hpd( pContext, 1 );
      fmc_imageon_wait_usec(1000000);

      //xil_printf("I2C Initializing - Config\n\r");
      fmc_imageon_iic_mux( pContext, FMC_IMAGEON_I2C_SELECT_HDMI_IN );
      fmc_imageon_iic_config3( pContext, iic_hdmi_in_config, IIC_HDMI_IN_CONFIG_LEN);

      //xil_printf("I2C Initializing - SPDIF Config\n\r");
      fmc_imageon_iic_mux( pContext, FMC_IMAGEON_I2C_SELECT_HDMI_IN );
      fmc_imageon_iic_config3( pContext, iic_hdmi_in_spdif_config, IIC_HDMI_IN_SPDIF_CONFIG_LEN);

      // sleep 100ms
      fmc_imageon_wait_usec(100000);
   }
...
*
******************************************************************************/
int fmc_imageon_hdmii_set_hpd( fmc_imageon_t *pContext, Xuint32 HotPlugStatus )
{
   Xuint8 reg_addr;
   Xuint8 reg_data;
   Xuint8 num_bytes;
   ...
   reg_addr = 0x01;
   num_bytes = pContext->pIIC->fpIicRead( pContext->pIIC, FMC_IMAGEON_IO_EXP_ADDR, reg_addr, &reg_data, 1); 
   //xil_printf( "[fmc_imageon_hdmii_set_hpd] PCA9534[0x%02X] <= 0x%02X\n\r", reg_addr, reg_data );
   if ( HotPlugStatus )
...
*
******************************************************************************/
int fmc_imageon_hdmii_set_rst( fmc_imageon_t *pContext, Xuint32 Reset )
{
   Xuint8 reg_addr;
   Xuint8 reg_data;
   Xuint8 num_bytes;
   ...
   reg_addr = 0x01;
   num_bytes = pContext->pIIC->fpIicRead( pContext->pIIC, FMC_IMAGEON_IO_EXP_ADDR, reg_addr, &reg_data, 1); 
   //xil_printf( "[fmc_imageon_hdmii_set_rst] PCA9534[0x%02X] <= 0x%02X\n\r", reg_addr, reg_data );
   if ( Reset )
...
*
******************************************************************************/
int fmc_imageon_hdmii_get_int( fmc_imageon_t *pContext, Xuint32 *pIntStatus )
{
   Xuint8 reg_addr;
   Xuint8 reg_data;
   Xuint8 num_bytes;
   ...
   reg_addr = 0x00;
   num_bytes = pContext->pIIC->fpIicRead( pContext->pIIC, FMC_IMAGEON_IO_EXP_ADDR, reg_addr, &reg_data, 1); 
   //xil_printf( "[fmc_imageon_hdmii_get_int] PCA9534[0x%02X] <= 0x%02X\n\r", reg_addr, reg_data );
...
int fmc_imageon_hdmii_get_lock( fmc_imageon_t *pContext )
{
   ...
   //xil_printf( "[fmc_imageon_hdmii_get_lock] ADV7611-HDMI4[0x%02X] <= 0x%02X\n\r", reg_addr, reg_0x07);
...
int fmc_imageon_hdmii_get_timing( fmc_imageon_t *pContext, fmc_imageon_video_timing_t *pTiming )
{
   ...
   memset( pTiming, 0x00, sizeof(fmc_imageon_video_timing_t) );
...
*
******************************************************************************/
int fmc_imageon_hdmio_init( fmc_imageon_t *pContext, Xuint32 Enable, fmc_imageon_video_timing_t *pTiming, Xuint32 WaitForHPD )
{
   Xuint8 i2c_dev;
   Xuint8 i2c_addr;
   Xuint8 i2c_data;
   ...
   if ( !Enable )
   {
      ...
      fmc_imageon_hdmio_set_pd( pContext, 1 );
   ...
   else
   {
      ...
      fmc_imageon_hdmio_set_pd( pContext, 0 );

      // Wait 100msec.
      fmc_imageon_wait_usec(100000);

      fmc_imageon_iic_mux( pContext, FMC_IMAGEON_I2C_SELECT_HDMI_OUT );
      ...
      fmc_imageon_iic_config3( pContext, iic_hdmi_out_config, IIC_HDMI_OUT_CONFIG_LEN);
      ...
      fmc_imageon_iic_config3( pContext, iic_hdmi_out_embedded_sync_config, IIC_HDMI_OUT_EMBEDDED_SYNC_CONFIG_LEN);

      // sleep 100ms
      fmc_imageon_wait_usec(100000);

      i2c_dev = FMC_IMAGEON_HDMI_OUT_ADDR; // ADV7511
      i2c_addr = 0x96;
      ...
      if ( WaitForHPD )
      {
         ...
         while (((i2c_data & 0xC0) != 0xC0) && (--timeout))
         {
            ...
            if (num_bytes < 1)
            {
               xil_printf( "[fmc_imageon_hdmio_init] ERROR occurred during I2C transaction !\n\r");
               timeout = 0;
               return 0;
            }
            //xil_printf("\t0x%02X[0x%02X] => 0x%02X\n\r", (i2c_dev << 1), i2c_addr, i2c_data);
         ...
         if (!timeout)
         {
            xil_printf( "[fmc_imageon_hdmio_init] ERROR - Timeout waiting for internal HotPlugDetect !\n\r");
            return 0;
         }
      } // if ( WaitForHPD )
   }
...
*
******************************************************************************/
int fmc_imageon_hdmio_set_pd( fmc_imageon_t *pContext, Xuint32 PowerDown )
{
   Xuint8 reg_addr;
   Xuint8 reg_data;
   Xuint8 num_bytes;
   ...
   reg_addr = 0x01;
   num_bytes = pContext->pIIC->fpIicRead( pContext->pIIC, FMC_IMAGEON_IO_EXP_ADDR, reg_addr, &reg_data, 1); 
   //xil_printf( "[fmc_imageon_hdmio_set_pd] PCA9534[0x%02X] <= 0x%02X\n\r", reg_addr, reg_data );
   if ( PowerDown )
...
*
******************************************************************************/
int fmc_imageon_hdmio_get_hpd( fmc_imageon_t *pContext, Xuint32 *pHotPlugDetect )
{
   Xuint8 reg_addr;
   Xuint8 reg_data;
   Xuint8 num_bytes;
   ...
   reg_addr = 0x00;
   num_bytes = pContext->pIIC->fpIicRead( pContext->pIIC, FMC_IMAGEON_IO_EXP_ADDR, reg_addr, &reg_data, 1); 
   //xil_printf( "[fmc_imageon_hdmio_get_hpd] PCA9534[0x%02X] <= 0x%02X\n\r", reg_addr, reg_data );
...
*
******************************************************************************/
int fmc_imageon_hdmii_read_edid( fmc_imageon_t *pContext, Xuint8 pData[256] )
{
   Xuint8 num_bytes = 0;
   int    idx;
...
*
******************************************************************************/
int fmc_imageon_hdmii_write_edid( fmc_imageon_t *pContext, Xuint8 pData[256] )
{
   Xuint8 num_bytes = 0;
   //Xuint8 dummy_data;
   int    idx;
   ...
   fmc_imageon_iic_config3( pContext, iic_hdmi_in_edid_pre, IIC_HDMI_IN_EDID_PRE_LEN);
   //
   for ( idx = 0; idx < 256; idx++ )
   {
...
*
******************************************************************************/
int fmc_imageon_hdmio_read_edid( fmc_imageon_t *pContext, Xuint8 pData[256] )
{
   Xuint8 num_bytes = 0;
   int    idx;
   
   // Mux Selection
   fmc_imageon_iic_mux( pContext, FMC_IMAGEON_I2C_SELECT_DDCEDID );

   // Read contents of EDID EEPROM
   for ( idx = 0; idx < 256; idx++ )
   {
      num_bytes += pContext->pIIC->fpIicRead( pContext->pIIC, FMC_IMAGEON_DDCEDID_ADDR, idx, &pData[idx], 1); 
   }
